...
document.addEventListener('DOMContentLoaded', function() {
    ...
    bulkActionForms.forEach(form => {
        ...
        function updateBulkActionButton() {
            const checkedCount = form.querySelectorAll('input[type="checkbox"][name="selected[]"]:checked').length;
            if (submitButton) {
    ...
    const filterInputs = document.querySelectorAll('input[data-filter], select[data-filter]');
    ...
    const fileUploads = document.querySelectorAll('input[type="file"][data-upload-progress]');
...
function showUploadProgress(fileCount) {
    ...
    if (progressContainer) {
        progressContainer.innerHTML = `
            ...
            <small class="text-muted">Uploading ${fileCount} file${fileCount > 1 ? 's' : ''}...</small>
        `;
